...
public class PluginClassLoader extends DexClassLoader {
    /**
     * apkfile ：加载apk/dex/jar的路径
     * optimizedDirectory：dex的输出路径(因为加载apk/jar的时候会解压除dex文件，这个路径就是保存dex文件的)
     * libraryPath：加载的时候需要用到的lib库，这个一般不用
     * systemClassLoader：DexClassLoader指定父加载器
     */
