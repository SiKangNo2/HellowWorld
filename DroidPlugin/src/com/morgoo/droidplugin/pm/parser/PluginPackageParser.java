...
public class PluginPackageParser {
    ...
    public PluginPackageParser(Context hostContext, File pluginFile) throws Exception {
        ...
        mParser.parsePackage(pluginFile, 0);//解析apk
        ...
        for (Object data : datas) {
            ...
            //获取插件Activity的IntentFilter
            List<IntentFilter> filters = mParser.readIntentFilterFromComponent(data);
            synchronized (mActivityIntentFilterCache) {
                //将Apk的IntentFilter加入缓存
                mActivityIntentFilterCache.remove(componentName);//删除同名
                mActivityIntentFilterCache.put(componentName, new ArrayList<IntentFilter>(filters));//加入最新的 Filter
            }
        }
        //得到服务节点信息
